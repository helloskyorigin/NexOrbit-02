'use client';

import React from 'react';
import { ConnectorSourceId } from './types';

export interface SourceIconProps {
  type: ConnectorSourceId | string;
  className?: string;
}

export const SourceIcon: React.FC<SourceIconProps> = ({ type, className = 'h-3.5 w-3.5' }) => {
  switch (type.toLowerCase()) {
    case 'gmail':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="none">
          <path
            d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"
            fill="#EA4335"
          />
          <path
            d="M22 6l-10 7L2 6"
            stroke="#ffffff"
            strokeWidth="1.8"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
          <path
            d="M2 18V6l10 7 10-7v12H2z"
            fill="#ffffff"
            fillOpacity="0.2"
          />
        </svg>
      );

    case 'calendar':
    case 'google calendar':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="none">
          <rect x="3" y="4" width="18" height="18" rx="3" fill="#4285F4" />
          <path d="M16 2v4M8 2v4M3 10h18" stroke="#ffffff" strokeWidth="2" strokeLinecap="round" />
          <text
            x="12"
            y="17"
            fill="#ffffff"
            fontSize="7.5"
            fontWeight="bold"
            textAnchor="middle"
            fontFamily="sans-serif"
          >
            31
          </text>
        </svg>
      );

    case 'drive':
    case 'google drive':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="none">
          <path d="M8.2 4h7.6l6.2 10.7-3.8 6.5H10.6L8.2 4z" fill="#0F9D58" />
          <path d="M1.8 15.2l3.8-6.5h14.6l-3.8 6.5H1.8z" fill="#FFBA00" />
          <path d="M8.2 4L4.4 10.5 1.8 15.2 5.6 21.2h7.6L8.2 4z" fill="#4285F4" />
        </svg>
      );

    case 'slack':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="none">
          <path d="M6 15a2 2 0 1 1-2-2h2v2z" fill="#E01E5A" />
          <path d="M7 15a2 2 0 0 1 4 0v5a2 2 0 1 1-4 0v-5z" fill="#E01E5A" />
          <path d="M9 6a2 2 0 1 1 2-2v2H9z" fill="#36C5F0" />
          <path d="M9 7a2 2 0 0 1 0 4H4a2 2 0 1 1 0-4h5z" fill="#36C5F0" />
          <path d="M18 9a2 2 0 1 1 2 2h-2V9z" fill="#2EB67D" />
          <path d="M17 9a2 2 0 0 1-4 0V4a2 2 0 1 1 4 0v5z" fill="#2EB67D" />
          <path d="M15 18a2 2 0 1 1-2 2v-2h2z" fill="#ECB22E" />
          <path d="M15 17a2 2 0 0 1 0-4h5a2 2 0 1 1 0 4h-5z" fill="#ECB22E" />
        </svg>
      );

    case 'asana':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="none">
          <circle cx="12" cy="7.5" r="4.5" fill="#F06A6A" />
          <circle cx="6" cy="16.5" r="4.5" fill="#F06A6A" />
          <circle cx="18" cy="16.5" r="4.5" fill="#F06A6A" />
        </svg>
      );

    case 'notion':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path d="M4 4.5A2.5 2.5 0 0 1 6.5 2h11A2.5 2.5 0 0 1 20 4.5v15a2.5 2.5 0 0 1-2.5 2.5h-11A2.5 2.5 0 0 1 4 19.5v-15zM7.5 6v12h2.2l4.8-7.5V18h2V6h-2.2L9.5 13.5V6h-2z" />
        </svg>
      );

    case 'github':
      return (
        <svg className={className} viewBox="0 0 24 24" fill="currentColor">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.53 1.032 1.53 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
          />
        </svg>
      );

    default:
      return null;
  }
};
